...
public final strictfp class FranceGeocentricInterpolationTest extends MathTransformTestCase {
     ...
     */
    @Test
    public void testGrid() throws IOException, FactoryException {
        final InputStream in = FranceGeocentricInterpolationTest.class.getResourceAsStream("GR3DF97A.txt");
        assertNotNull("Test file \"GR3DF97A.txt\" not found.", in);
        final FranceGeocentricInterpolation.Grid grid;
        try (final BufferedReader r = new BufferedReader(new InputStreamReader(in))) {
            grid = new FranceGeocentricInterpolation.Grid(FranceGeocentricInterpolation.Grid.readHeader(r, null));
            grid.load(r, null);
        }
        /*
         * Verify envelope.
