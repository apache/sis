...
abstract class Transferer {
     ...
     * instead than SampleModel.getSamples(…, float[]). The reason is that the former method is optimized
    ...
    /**
     * Returns {@code true} if the given raster use a data type than can be casted to the {@code float} type
