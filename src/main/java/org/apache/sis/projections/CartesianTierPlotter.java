 ...
 * 
 * 
 * A plotter handling Cartesian tiers.
 * 
 * @author pjaol
 * @author mattmann
 * @version $Revision$
...
public class CartesianTierPlotter {
  ...
  public CartesianTierPlotter(int tierLevel, Projection projector) {
  ...
  private void setTierVerticalPosDivider() {
    ...
    tierVerticalPosDivider = new Double(Math.ceil(Math.log10(new Integer(
        this.tierLength).doubleValue()))).intValue();

    // 
    tierVerticalPosDivider = (int) Math
        .pow(10, (double) tierVerticalPosDivider);
   ...
   * longitude box id are transposded in to position coordinates.
   * 
   * @param latitude
   * @param longitude
   * @return
  ...
  /**
   * a log to the base 2 formula <code>Math.log(value) / Math.log(2)</code>
   * 
   * @param value
   * @return
   */
