...
final class GeoKeysWriter {
    ...
    GeoKeysWriter(final GeoTiffStore store) {
        ...
        keyDirectory = new short[(GeoKeys.NUM_KEYS + 1) * GeoKeysLoader.ENTRY_LENGTH];
     ...
     */
    final void write(final GridGeometry grid, final MetadataFetcher<?> metadata)
        ...
        for (int i = cs.getDimension(); --i >= 0;) {
            ...
            if (type != null) {
                ...
                if (previous != null && !previous.equals(unit)) {
                    warning(store.errors().getString(Errors.Keys.HeterogynousUnitsIn_1, name(cs)), null);
                }
            } else {
    ...
    private void writeName(final short key, final String type, final IdentifiedObject object) {
        ...
        citationLengthIndex = keyCount * GeoKeysLoader.ENTRY_LENGTH + 2;        // Length is the field #2.
    }
    ...
    private void appendName(final String type, final IdentifiedObject object) {
        ...
        if (name != null) {
            ...
            final int limit = keyCount * GeoKeysLoader.ENTRY_LENGTH;    // Inclusive.
            i++;                                                        // Offset is the field after length.
            while (i < limit) {
                i += GeoKeysLoader.ENTRY_LENGTH;                        // Really after (i < limit) test.
    ...
    private short getGeoCode(final IdentifiedObject object) throws FactoryException {
        ...
        if (id != null) try {
            ...
            warning(store.errors().getString(Errors.Keys.CanNotParse_1, IdentifiedObjects.toString(id)), e);
        }
    ...
    final short[] keyDirectory() {
        ...
        return ArraysExt.resize(keyDirectory, (keyCount + 1) * GeoKeysLoader.ENTRY_LENGTH);
    }
