...
final class GridMapping {
    ...
    private boolean parseProjectionParameters() throws FactoryException, TransformException {
        ...
        DefaultDatumEnsemble<GeodeticDatum> ensemble = null;
        ...
        if (hasBuiltSomeCustomObjects) {
            final Map<String,?> properties = properties(definition, wantGeographicCRS, Convention.GEOGRAPHIC_CRS_NAME, () -> {
                ...
                if (base == null) {
                    base = datum;
                } else if (base instanceof GeneralDerivedCRS) {
                    base = ((GeneralDerivedCRS) fromWKT).getBaseCRS();
                }
                return base.getName();
            });
            baseCRS = crsFactory.createGeographicCRS(properties, (datum != null) ? datum : (GeodeticDatum) ensemble, baseCRS.getCoordinateSystem());
        }
